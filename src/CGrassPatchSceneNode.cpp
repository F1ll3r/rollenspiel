...
u32 CGrassPatchSceneNode::getWindRes()
{
	return windGridRes;
}


video::SMaterial& CGrassPatchSceneNode::getMaterial(u32 i)
{
	return Material;
}


u32 CGrassPatchSceneNode::getMaterialCount() const
{
	return 1;
}


core::dimension2d<s32> CGrassPatchSceneNode::getImageCount()
{
	return imagecount;
}
...
bool CGrassPatchSceneNode::Create() 
{
 ...
 for(s32 i=0; i<count; ++i)
 {
	...
	s32 x1 = core::floor32( xz.X );
	s32 z1 = core::floor32( xz.Z ); 
	
	// drop blades that are outside the terrain
	if ( x1 < 1 || 
		 z1 < 1 || 
		 x1 > (s32)TerrainHeightMap->getDimension().Width - 1 || 
		 z1 > (s32)TerrainHeightMap->getDimension().Height - 1 ) 
	{
		 --count; --i;
		 Particles.set_used(count);
		 continue;
	...
	video::SColor cDensity  = TerrainGrassMap->getPixel(TerrainGrassMap->getDimension().Width - x1, z1);
	...
	core::dimension2d<f32> size((f32)(rand() % 50 + 20), 100.0f);
	size.Height *= (f32)cDensity.getBlue()/200.0f;
	
	Particles[i].flex  = size.Height / 120.0f;
	Particles[i].pos.Y = Terrain->getHeight(p.X, p.Z) + (size.Height*0.5f);
